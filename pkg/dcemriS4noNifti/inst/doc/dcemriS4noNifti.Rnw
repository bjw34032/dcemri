...
The \pkg{dcemriS4noNifti} package requires incoming data to be in
the ANALYZE~7.5 or NIfTI formats.  Data conversion (e.g., from DICOM
to NIfTI) must be performed by the user before \pkg{dcemriS4noNifti} may be
...
At this point in time the \pkg{dcemriS4noNifti} package cannot apply an
...
(neurological).  Future versions of \pkg{dcemriS4noNifti} will attempt to
...
A major improvement in the \pkg{dcemriS4noNifti} package is the fact that
...
utilized in the \pkg{dcemriS4noNifti} package.  First, extensions are
...
to turn off the ``audit trail'' option in \pkg{dcemriS4noNifti}.
...
<<zstat1>>=
zstat1 <- readNIfTI(system.file("nifti/zstat1.nii.gz", package="dcemriS4noNifti"))
zstat1
@ 
