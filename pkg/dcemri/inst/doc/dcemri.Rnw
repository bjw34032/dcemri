...
and \cite{ort-etal:efficient}.  \pkg{dcemri} has incorporated two of
...
\begin{eqnarray*}
  ...
  \end{array}\right.
\end{eqnarray*}
\citep{ort-etal:efficient} into the kinetic parameter estimation step.
While default values for each model are provided, there is also the
ability to include user-specified parameters.
