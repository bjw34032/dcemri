...
The NIfTI format contains an implicit generalised spatial transformation from
the data co-ordinate system $(i,j,k)$ into a real-space ``right-handed''
co-ordinate system. In this real-space system, the axes $(x,y,z)$ are
\emph{usually} set such that $x$ increases from left to right, $y$ increases
from posterior to anterior and $z$ increases from inferior to superior.

Unfortunately, this package cannot properly transform data into the
$(x,y,z)$ space; not least because the $(x,y,z)$ space could require
non-integral indices and hence require a more complicated accessor and imaging
system. We do, however, attempt to at least partially transform the data, before
setting the $x$ axis such that it increases from right to left, as per the
standard convention. A more compliant mechanism is planned for future versions of this package.
...
dim(mni)
@ 
<<figure1,echo=FALSE,results=hide>>=
png(filename="avg152T1_LR_nifti.png", width=400, height=400)
X <- nrow(mni) ; Y <- ncol(mni) ; Z <- nsli(mni)
zrange <- range(mni)
...
This may be displayed in a $10{\times}10$ grid of images
(Figure~\ref{fig:avg152T1_LR_nifti}) using the below R code.

<<avg152T1_LR_nifti-example,eval=FALSE>>=
zrange <- range(mni)
par(mfrow=c(10,10), mar=rep(0,4))
for (z in 1:Z) {
  image(1:nrow(mni), 1:ncol(mni), mni[,,z,1], zlim=zrange, col=grey(0:64/64),
        xlab="", ylab="", axes=FALSE)
}
@
...
dim(mni)
@ 
<<figure2,echo=FALSE,results=hide>>=
...
dim(ffd)
@ 
<<figure3,echo=FALSE,results=hide>>=
png(filename="ffd.png", width=400, height=400)
X <- nrow(ffd) ; Y <- ncol(ffd) ; Z <- nsli(ffd) ; W <- ntim(ffd)
zrange <- range(ffd)
w <- 1
...
(\texttt{filtereted\_func\_data.nii} and \texttt{zstat1.nii}) were
...
dim(zstat1)
@ 
<<figure4,echo=FALSE,results=hide>>=
...
for (z in 1:Z) {
  image(1:X, 1:Y, ffd[,,z,w], zlim=zrange, col=grey(0:64/64), 
        xlab="", ylab="", axes=FALSE)
  image(1:X, 1:Y, ifelse(abs(zstat1[,,z,1]) > 4, zstat1[,,z,1], NA),
...
There are a miriad of techniques to quantify T1 using MRI.  Currently
...
<<doubleanglemethod>>=
sdam60 <- read.img("SDAM_ep2d_60deg_26slc.nii.gz")
dim(sdam60)
sdam120 <- read.img("SDAM_ep2d_120deg_26slc.nii.gz")
dim(sdam120)
sdam.image <- rowMeans(dam(sdam60, sdam120, 60), dims=3)
mask <- rowSums(sdam60, dims=3) > 500
# sdam.image[is.na(sdam.image)] <- 0 
@ 
<<figure5,echo=FALSE,results=hide>>=
Z <- 26 # nsli(sdam60)
w <- 5
SDAM <- drop(read.img("SDAM_smooth.nii.gz"))
...
Gaussian smoothing) for a T1 phantom containg a variety of T1
...
(\code{SDAM}), multiple flip-angle acquistions can be used to estimate
...
fanglesB1 <- fangles * array(SDAM[,,1:26], c(X,Y,Z,nangles))
R1 <- R1.fast(flip, mask[,,1:26], fanglesB1, TR, verbose=TRUE)
@ 
<<figure6,echo=FALSE,results=hide>>=
...
Figure~\ref{fig:t1-phantom} displays the quantitative T1 map for the
phantom using information from the estimated B1+ field.  

\begin{figure}[!htbp]
  \centering
  \includegraphics{t1_phantom.png}
...
\begin{figure}[!htbp]
\begin{center}
<<figure7,fig=TRUE,echo=FALSE>>=
T1 <- c(.484,.350,1.07,.648,.456,1.07,.660,1.543,1.543,.353)
par(mfrow=c(1,1), mar=c(5,4,4,2)+.1)
boxplot(split(1/drop(R1$R10)[,,-9:9+13], as.factor(drop(pmask)[,,-9:9+13]))[-1], 
...
around the phantom, clockwise starting from approximatley one o'clock.
...
cannulation (\url{en.wikipedia.org/wiki/Cannula}) on the subject in
order to characterize the arterial input function (AIF), it has been
common to use literature-based AIFs in the DCE-MRI literature.
Examples include 
\begin{equation*}
...
to the observations.  Recent models include \cite{par-etal:derived}
...
\begin{eqnarray*}
  ...
  %\end{array}\right.
\end{eqnarray*}
which can be fitted to the data obtained through acquistion using the linear
regression. The \code{orton.exp.lm} function provides this capability and is
shown below:

<<buckley.aif>>=
data("buckley")
aifparams <- with(buckley, orton.exp.lm(time.min, input))
pseudoobservedaif <- with(buckley,aif.orton.exp(tt=time.min, AB=aifparams$AB, 
                                   AG=aifparams$AG, muB=aifparams$muB,
                                   muG=aifparams$muG))
@
\begin{figure}[!htbp]
\begin{center}
<<figure8,fig=TRUE,echo=FALSE>>=
with(buckley, plot(time.min, input,type="l",lty=2))
with(buckley, lines(time.min, pseudoobservedaif,lty=1))
legend(3,4,legend=c("Observed arterial concentration", "Our fit"), lty=c(2,1))
@ 
\end{center}
...
\pkg{dcemri}.  We recommend one uses builtin facilities in
