...
\pkg{oro.nifti} package \citep{oro.nifti}, where both Analyze and NIfTI data
are represented in S4 classes.  Parameter estimates in \pkg{dcemriS4}
inherit attributes from the incoming Analyze/NIfTI objects in order to
preserve anatomical and physiological information for appropriate
visualization in \proglang{R} or other software packages.
...
reader will find several relevant chapters in
\citet{jac-buc-par:dce-mri} pertaining to the scientific background,
motivation and application of DCE-MRI.

[{\bf Diffustion Weighted Imaging}]



\section[Dynamic Contrast-enhanced Magnetic Resonance Imaging]{Dynamic Contrast-enhanced Magnetic Resonance Imaging}

\subsection[T1 Relaxation and Gadolinium Concentration]{T1 Relaxation rate and Gadolinium Concentration}
...
flip angle.  Note, the transverse relaxation time was
$\TR=4.22$~ms in the example data acquisition.  The
Levenberg-Marquardt algorithm, provided in \pkg{minpack.lm}
\citep{minpack.lm}, was used to estimate the parameters; see a recent
discussion by \cite{ahe-etal:levenburg-marquardt}.

Estimate the post-injection longitudinal relaxation rate $R_1(t)$
using the signal intensity from pre- and post-contrast acquisitions
via
\begin{eqnarray}
...
\end{eqnarray}
where $\theta=15^\circ$ was used in the dynamic sequence in the
example data set.
...
calls \code{E10.lm}.  Conversion of the dynamic signal intensities to
contrast agent concentration is performed via
\begin{equation}
  C_t(t) = \frac{1}{r_1}\left(\frac{1}{T_1} - \frac{1}{T_{10}}\right),\label{eqn:conc}
\end{equation}
...
For {\em in vivo} MRI at high field ($\geq{3}\text{Tesla}$) it is
...
exciting or manipulating large collections of spins, non-uniformity in
B1+ results in nonuniform treatment of spins.  This leads to a
spatially varying image signal and image contrast, and to difficulty
in image interpretation and image-based quantification
...
with prescribed tip $\alpha_1$ and $I_2$ with prescribed tip
...
AIFs in the DCE-MRI literature.  Examples include
\begin{equation*}
...
\citep{ort-etal:efficient}.  The \pkg{dcemriS4} package has
incorporated one of these parametric models
\begin{equation*}\label{eqn:orton}
  C_p(t) = A_B t e^{-\mu_Bt} + A_G \left( e^{-\mu_Gt} + e^{-\mu_Bt}\right)
\end{equation*}
\citep{ort-etal:efficient}, which may be fitted to the observed data
...
this capability using the common double-exponential parametric form
Eqn.~\ref{eqn:orton}.
...
\begin{eqnarray}
  C_t(t) &=& \ktrans \left[ C_p(t) \otimes e^{-\kep t} \right], \label{eqn:kety}\\
  C_t(t) &=& \vp C_p(t) + \ktrans \left[ C_p(t) \otimes e^{-\kep t} \label{eqn:extendedkety}
    \right].
\end{eqnarray}
Here, $\ktrans$ represents the volume transfer constant between extracellular extravascular space (EES) per minute, and $\kep$ is the rate constant between EES and blood plasma. The parameter $\vp$ describes the fraction of plasma in the tisse, and $v_e=\ktrans/kep$ can be interpreted as fraction of EES.

Parameter estimation is achieved using one of three options in the
current version of this software:
\begin{itemize}
\item \code{dcemri.lm}: Non-linear regression using non-linear least
  squares (Levenburg-Marquardt optimization),
...
Least-square estimates of the kinetic parameters $\ktrans$ and
$\kep$ (also $\vp$ for the extended Kety model) are provided in
\code{dcemri.lm} and maximum a posteriori estimates in \code{dcemri.map}.
With \code{dcemri.bayes} the posterior median
is provided as summary statistic, estimated via Markov chain Monte Carlo.
All samples from the joint posterior distribution may be provided using 
\code{samples=TRUE}, allowing one to interrogate the posterior probability
density function (PDF) of the parameter estimates, which can be used, e.g.,
to compute credible intervals. Given estimated kinetic parameters, 
\code{kineticModel} and \code{orton.exp.lm} can be used to compute 
constrast agent concentration for specific models. 

For the Bayesian approaches (\code{dcemri.bayes}, \code{dcemri.map}
hyper prior parameters have to be specified for the prior 
distributions on the kinetic parameters. Following 
\citep{sch-etal:TMI}, Gaussian priors on $\log(\ktrans)$ and
$\log(\kep)$, a Beta prior on $\vp$, 
and a Gamma prior on the variance of the observation error are used.

As alternative to parametric modelling, \code{dcemri.spline} can be
used to de-convolve and de-noise the contrast agent concentration
cureve using an adaptive penalized Spline approach \citep{sch-etal:TMI09}.
Kinetic parameters can however be estimated by fitting the response model
to the estimated response curve; e.g., an exponential in case of a Weinman model.
\code{dcemri.spline} also uses a Bayesian approach, i.e., smoothing hyperpriors
have to be defined and posterior pdfs can be computed for all results. 

Altough all of these function can also be used for aggregated data,
e.g., a mean concentration curve in a region of interest, they are
intended to be used on a voxel basis. As voxel-by-voxel can be
time-consuming, \pkg{dcemriS4} supports parallel computing using
the \pkg{multicore} package. The computation can be parallelized 
with \code{multicore=TRUE}.
...
\pkg{PILFER} (\url{http://pilfer.sourceforge.net}).
...
national repository integrating \textit{in vivo} cancer images with
...
Functions of the \pkg{oro.nifti} package are used to read the raw 
MR signal intensity files. In this example pre-contrast multiple flip 
angle data are available for estimation of contrast agent concentration.
We use \code{CA.fast} to estimate the intrinsic relaxation rate
$R_10$ and $m_0$ from Eqn.\ref{eqn:signal} and the contrast agent concentration
curve $C_t(t)$ from Eqn.\ref{eqn:conc}. In order to save memory, we only use
four slices of the data.

<<RIDER_Neuro_MRI,eval=FALSE,echo=TRUE>>=
...
we want to fit a kinetic model to these curves. Here, we use the "extended"
Kety model, which includes a vascular compartment. An arterial input 
function has to be defined, here we use the standard AIF, sum of two
exponentials, with values taken from \cite{tof-ker:measurement}. 
A numeric optimization of the least square fit using the Levenberg-Marquardt
algorithm is provided by \code{dcemri.lm()}:

<<RIDER_Neuro_MRI+lm,eval=FALSE,echo=TRUE>>=
acqtimes <- str2time(unique(sort(scan("rawtimes.txt", quiet=TRUE))))$time
conc <- readNIfTI(paste(perf, "gdconc", sep="_"))
fit.lm <- dcemri.lm(ca$conc, (acqtimes - acqtimes[9]) / 60, mask, 
                 model="extended", aif="fritz.hansen", verbose=TRUE,
                 multicore=TRUE)
...
perf <- paste("281949", "19040721", "perfusion", sep="_")
fit.lm=list()
fit.lm$ktrans <- readNIfTI(paste(perf, "ktrans", sep="_"))
dynamic <- readNIfTI(perf)
dynamic <- dynamic[,,5:8,]
@ 
<<RIDER_Neuro_MRI+lm4,eval=FALSE,echo=TRUE>>=
overlay(as(dynamic, "nifti"), 
        ifelse(fit.lm$ktrans < .1, fit.lm$ktrans, NA),
        w=11, zlim.x=c(32,512), zlim.y=c(0,.1))
@ 
<<RIDER_Neuro_MRI+lm5,eval=FALSE,echo=FALSE>>=
dev.off()
fit.ktrans.lm <- readNIfTI(paste(perf, "ktrans", sep="_"))
fit.kep.lm <- readNIfTI(paste(perf, "kep", sep="_"))
fit.vp.lm <- readNIfTI(paste(perf, "vp", sep="_"))
fit.ve.lm <- fit.ktrans.lm/fit.kep.lm
fit.sse.lm <- readNIfTI(paste(perf, "sse", sep="_"))
yy<-25:208
xx<-37:220
png(file=paste(paste(perf, "kep", sep="_"), "png", sep="."), 
    width=480, height=480)
overlay(as(dynamic[xx,yy,,], "nifti"), 
        ifelse(fit.kep.lm[xx,yy,] < 1.25, fit.kep.lm[xx,yy,], NA),
        w=11, z=3, zlim.x=c(32,512), zlim.y=c(0,1.25), plot.type="single")
dev.off()
png(file=paste(paste(perf, "vp", sep="_"), "png", sep="."), 
    width=480, height=480)
overlay(as(dynamic[xx,yy,,], "nifti"), 
        ifelse(fit.vp.lm[xx,yy,] < 0.3, fit.vp.lm[xx,yy,], NA),
        w=11, z=3, zlim.x=c(32,512), zlim.y=c(0,.03), plot.type="single")
dev.off()
png(file=paste(paste(perf, "ve", sep="_"), "png", sep="."), 
    width=480, height=480)
overlay(as(dynamic[xx,yy,,], "nifti"), 
        ifelse(fit.ve.lm[xx,yy,] < .3, fit.ve.lm[xx,yy,], NA),
        w=11, z=3, zlim.x=c(32,512), zlim.y=c(0,.3), plot.type="single")
dev.off()
png(file=paste(paste(perf, "sse", sep="_"), "png", sep="."), 
    width=480, height=480)
overlay(as(dynamic[xx,yy,,], "nifti"), 
        ifelse(fit.sse.lm[xx,yy,] < .04, fit.sse.lm[xx,yy,], NA),
        w=11, z=3, zlim.x=c(32,512), zlim.y=c(0,.04), plot.type="single",main="SSE")
dev.off()
@ 

\begin{figure}[!htbp]
  \begin{center}
    \includegraphics*[width=0.75\textwidth]{281949_19040721_perfusion_ktrans.png}
  \end{center}
  \caption{$\ktrans$ parameter maps for four slices of the RIDER example.
    Two potential tumours are visible with ring-shaped increased $\ktrans$ 
    and centers of decreased $\ktrans$.
...
\begin{figure}[!htbp]
  ...
  \begin{tabular}{ccc}
    $\kep$ & $v_e$ & $v_p$ \\    
    \includegraphics*[width=0.3\textwidth]{281949_19040721_perfusion_kep.png}&
    \includegraphics*[width=0.3\textwidth]{281949_19040721_perfusion_ve.png}&
    \includegraphics*[width=0.3\textwidth]{281949_19040721_perfusion_vp.png}
    \end{tabular}
  \end{center}
  \caption{Parameter maps for central tumour slice of the RIDER data. 
    From left to right: $\kep$ map; efflux is reduced in the area of both tumours.
    $v_e$ map; increased value on tumour rims. 
    $v_p$ map; apparently no plasma in the center of both tumours.
...
\begin{figure}[!htbp]
  \begin{center}
    \includegraphics*[width=0.3\textwidth]{281949_19040721_perfusion_sse.png}
  \end{center}
  \caption{Map of sum of squared error of fit per voxel. Qualitity of fit is equal over the tumour region.}
  \label{fig:perfusion-sse}
\end{figure}

Fig.~\ref{fig:perfusion-ktrans} shows the estimated $\ktrans$ parameter maps
for four slices. Two rings of increased $\ktrans$ values 
can be identified from the parameter maps, which indicates the presence of
two tumours. In both cases $\ktrans$ is nearly zero in the centre 
of both rings, indicating that no blood is supplied to the probably necrotic
core of the tumours. 
Fig.~\ref{fig:perfusion-parametermaps} depicts parameter maps of slice 7 
(bottom left in Fig.~\ref{fig:perfusion-ktrans}) for the other kinetic parameters.
All maps strengthen the finding of two tumours with necrotic cores. For both tumours
the fraction of extravascular extracellular space (EES) $v_e$ is increased at
the rim, which indicates that normal tissue was destroyed here. There is nearly no
perfusion in the tumours, i.e., low values of $\kep$ and $v_p$, indicating
necrotic cores. Fig.~\ref{fig:perfusion-sse}
shows sum of squared error SSE of the fit to the kinetic model. This image can be used as
quality check. Here, for example, the region of interest covers cerebrospinal 
fluid, scull, and air, for all of which the kinetic model is not appropriate and, hence,
SSE is high. However, across the healthy and the tumour tissue goodness of fit is
nearly invariant.

As all non-linear regression algorithms, the Levenberg-Marquardt algorithm
used in \code{dcemri.lm} is not guaranteed to converge and is quite susceptible 
to noise. More robust results can be achieved by using biological prior 
information in a Bayesian framework \citep{sch-etal:TMI}.
Two possibilities to gain parameter estimates from a Bayesian model
are implemented in \pkg{dcemriS4}. Whereas \code{dcemri.bayes} uses
a Markov Chain Monte Carlo (MCMC) algorithm to explore the posterior 
pdf, \code{dcemri.map} uses numerical optimization in order to maximize the 
posterior. 

For neurological DCE-MRI data, $\ktrans$ values are typically roughly in the 
range of $0 - 0.1$, and $\kep$ values are in the range of $0 - 1.25$ (see above). 
Here, we use a Gaussian distribution with mean $\log(0.05)$ and variance $1$
on $\log(\ktrans)$, and mean $\log(0.7)$ and variance $1$ on $\log(\kep)$ as priors.
For $v_p$, we use Beta-distribution with $a=1, b=19$, i.e., an a prior expected value of 0.5.

<<RiderNeuroMRI+map,eval=FALSE,echo=FALSE>>=
perf <- paste("281949", "19040721", "perfusion", sep="_")
acqtimes <- str2time(unique(sort(scan("rawtimes.txt", quiet=TRUE))))$time
conc <- readNIfTI(paste(perf, "gdconc", sep="_"))
mask <- readANALYZE(paste(perf, "mask", sep="_"))
...
mask<-mask[,,5:8]
@ 
<<RiderNeuroMRI+map,eval=FALSE,echo=TRUE>>=
fit.map <- dcemri.map(conc, (acqtimes - acqtimes[9]) / 60, mask, 
                    model="extended", aif="fritz.hansen",
                    ab.ktrans=c(log(0.05),1),ab.kep=c(log(.7),1),ab.vp=c(1,19),
...
dynamic <- dynamic[,,5:8,]
fit.map<-list()
fit.map$ktrans <- readNIfTI(paste(perf, "ktrans", "map", sep="_"))
yy<-25:208
xx<-37:220
png(file=paste(paste(perf, "ktrans", "map", sep="_"), "png", sep="."), 
    width=480, height=480)
overlay(as(dynamic[xx,yy,,], "nifti"), 
        ifelse(fit.map$ktrans[xx,yy,] < .1, fit.map$ktrans[xx,yy,], NA),
        w=11, zlim.x=c(32,512), zlim.y=c(0,.1), plot.type="single", z=3)
dev.off()
fit.lm<-list()
fit.lm$ktrans <- readNIfTI(paste(perf, "ktrans", sep="_"))
png(file=paste(paste(perf, "ktrans", "compare", sep="_"), "png", sep="."), 
    width=480, height=480)
plot(fit.lm$ktrans,fit.map$ktrans,xlim=c(0,.3),ylim=c(0,.3),xlab="Ktrans Levenberg-Marquardt", ylab="Ktrans Maximum A Posteriori",pch=19)
lines(c(0,100),c(0,100),col="darkgrey",lty=2)
dev.off()
@ 

\begin{figure}[!htbp]
\begin{center}
    \includegraphics*[width=0.375\textwidth]{281949_19040721_perfusion_ktrans_map.png}
\end{center}
  \caption{Maximum a posteriori $\ktrans$ parameter map for central tumour slice of the RIDER DCE-MRI data.
    \label{fig:perfusion-map}}
\end{figure}

Fig.~\ref{fig:perfusion-map} depicts the $\ktrans$ parameter map for the
central tumour slice of the RIDER example (bottom left slice in 
Fig.~\ref{fig:perfusion-ktrans}) gained by the maximum a posterior estimator.
Estimated values are roughly the same as with the Levenberg-Marquardt 
algorithm. However, the estimation is a little bit more robust.
Here, by using biological prior information the number of voxel where
the algorithm did not converge is reduced by a fifth:
<<RiderNeuroMRI+map6,eval=TRUE,echo=FALSE>>=
perf <- paste("281949", "19040721", "perfusion", sep="_")
acqtimes <- str2time(unique(sort(scan("rawtimes.txt", quiet=TRUE))))$time
conc <- readNIfTI(paste(perf, "gdconc", sep="_"))
mask <- readANALYZE(paste(perf, "mask", sep="_"))
...
fit.map$ktrans <- readNIfTI(paste(perf, "ktrans", "map", sep="_"))
@
<<RiderNeuroMRI+map7,eval=TRUE,echo=TRUE>>=
x<-apply(mask,1,sum)!=0
y<-apply(mask,2,sum)!=0
print(c(sum(mask[x,y,]),sum(is.na(fit.lm$ktrans[x,y,])),sum(is.na(fit.map$ktrans[x,y,]))))
@ 

Computation times for \code{dcemri.lm} and \code{dcemri.lm} are roughly
the same. Using the MCMC algorithm provided by \code{dcemri.bayes} is much more
computational intensive. However, the MCMC algorithm explores the complete
posterior pdf. Hence, random samples of the posteriori pdf are given as a result
(with \code{samples=TRUE}). From this, statistics about the reliability of the estimated
parameters can be gained. For example, here we use the standard deviation of the
$\ktrans$ parameter. 

<<RiderNeuroMRI+Bayes0,eval=FALSE,echo=FALSE>>=
perf <- paste("281949", "19040721", "perfusion", sep="_")
acqtimes <- str2time(unique(sort(scan("rawtimes.txt", quiet=TRUE))))$time
conc <- readNIfTI(paste(perf, "gdconc", sep="_"))
mask <- readANALYZE(paste(perf, "mask", sep="_"))
...
x <- apply(mask,1,sum)!=0
y <- apply(mask,2,sum)!=0
fit.bayes <- dcemri.bayes(conc[x,y,3,], (acqtimes - acqtimes[9]) / 60,
                array(TRUE,c(sum(x),sum(y))),
                model="extended", aif="fritz.hansen", 
                multicore=TRUE, verbose=TRUE, samples=TRUE,
                ab.ktrans=c(log(.05),1),ab.kep=c(log(.7),1), ab.vp=c(1,19))
...
@ 
<<RiderNeuroMRI+Bayes,eval=FALSE,echo=TRUE>>=
fit.bayes <- dcemri.bayes(conc, (acqtimes - acqtimes[9]) / 60, mask,
                model="extended", aif="fritz.hansen", 
                multicore=TRUE, verbose=TRUE, samples=TRUE,
                ab.ktrans=c(log(.05),1),ab.kep=c(log(.7),1), ab.vp=c(1,19))
writeNIfTI(fit.bayes$ktrans, paste(perf, "ktrans","bayes", sep="_"))
...
dynamic <- readNIfTI(perf)
dynamic <- dynamic[,,5:8,]
ktrans.bayes<-readNIfTI(paste(perf, "ktrans","bayes", sep="_"))
...
for (i in 1:256)for (j in 1:256)if(x[i])if(y[j])ktrans[i,j,2]<-ktrans.bayes[i-min(which(x))+1,j-min(which(y))+1,1]
png(file=paste(paste(perf, "ktrans", "bayes", sep="_"), "png", sep="."), 
    width=480, height=480)
yy<-25:208
xx<-37:220
overlay(as(dynamic[xx,yy,,], "nifti"), ifelse(ktrans[xx,yy,] < .1, ktrans[xx,yy,], NA), 
        w=11, zlim.x=c(32,512), zlim.y=c(0,.1), plot.type="single", z=3)
dev.off()
sd.bayes<-readNIfTI(paste(perf, "ktrans","bayes","sd", sep="_"))
sd<-ktrans
for (i in 1:256)for (j in 1:256)if(x[i])if(y[j])sd[i,j,3]<-sd.bayes[i-min(which(x))+1,j-min(which(y))+1]
yy<-25:208
xx<-37:220
...
overlay(dynamic[xx,yy,,], 
        ...
        w=11, zlim.x=c(32,512), zlim.y=c(0,.005), plot.type="single", z=3)
dev.off()
png(file=paste(paste(perf, "ktrans", "bayes", "sdktrans", sep="_"), "png", sep="."), 
    width=480, height=480)
overlay(dynamic[xx,yy,,], 
        ifelse(sd[xx,yy,]/ktrans[xx,yy,] < 0.1, sd[xx,yy,]/ktrans[xx,yy,], NA),
        w=11, zlim.x=c(32,512), zlim.y=c(0,.1), plot.type="single", z=3)
dev.off()
@
...
\begin{figure}[!htbp]
  \begin{center}
\begin{tabular}{ccc}
Median $\ktrans$ & Standard deviation $\ktrans$ & Standard deviation/median\\ 
    \includegraphics*[width=0.3\textwidth]{281949_19040721_perfusion_ktrans_bayes.png}&
    \includegraphics*[width=0.3\textwidth]{281949_19040721_perfusion_ktrans_bayes_sd.png}&
    \includegraphics*[width=0.3\textwidth]{281949_19040721_perfusion_ktrans_bayes_sdktrans.png}
  \end{tabular}
  \end{center}
  \caption{From left to right: Median posterior $\ktrans$ parameter map. Map of standard 
    deviation of $\ktrans$ posterior distribution. Map of standard deviation divided by
    estimated $\ktrans$ value. \label{fig:perfusion-bayes}}
\end{figure}

Fig.~\ref{fig:perfusion-bayes} depicts the $\ktrans$ parameter map estimated
as the median of the marginal $\ktrans$ posterior pdf. Please note, that in general the
median posterior value is different from the maximum a posterior estimator
computed by \code{dcemri.map}. However, $\ktrans$ values in Fig.~\ref{fig:perfusion-bayes}
are similar to those estimated by \code{dcemri.lm} and \code{dcemri.map}.
Fig.~\ref{fig:perfusion-bayes} also shows the $\ktrans$ standard deviation map
and a map of standard deviation divided by estimated $\ktrans$ values. 
As can be seen from the maps, the standard deviation of $\ktrans$ is increased
in the area of increased $\ktrans$, even after adjusting for the estimated $\ktrans$ value.
However, in general the standard deviation is quite low, below $0.1$, in the area of
the tumour.

As alternative to fit a kinetic model, \code{dcemri.spline}
allows to fit a smooth, non-parametric curve to the data. 
Smoothness of the curve and fit to the data are controlled by two 
Gamma priors: the prior for the adaptive smoothness parameters (\code{ab.hyper}) and
the prior for the variance of the observation error (\code{ab.tauepsilon}). 
A method to compute the onset of contrast agent concentration is also implemented in
\code{dcemri.spline}. Further details can be found in \cite{sch-etal:TMI09}.
The function gives not only the fit to the data, but also the deconvolved response 
function in each voxel.

<<RiderNeuroMRI+Spline0,eval=FALSE,echo=FALSE>>=
perf <- paste("281949", "19040721", "perfusion", sep="_")
acqtimes <- str2time(unique(sort(scan("rawtimes.txt", quiet=TRUE))))$time
conc <- readNIfTI(paste(perf, "gdconc", sep="_"))
mask <- readANALYZE(paste(perf, "mask", sep="_"))
...
fit.spline <- dcemri.spline(conc[x,y,3,-(1:8)], (acqtimes - acqtimes[9])[-(1:8)] / 60,
array(TRUE,c(sum(x),sum(y))), 
model="weinmann", aif="fritz.hansen", 
ab.tauepsilon=c(1/10,1/1000), ab.hyper=c(0.01,0.01),
multicore=TRUE, samples=FALSE, nlr=TRUE)
@ 
<<RiderNeuroMRI+Spline2,eval=FALSE,echo=FALSE>>=
...
perf <- paste("281949", "19040721", "perfusion", sep="_")
dynamic <- readNIfTI(perf)
ktrans.spline<-readNIfTI(paste(perf, "ktrans","spline", sep="_"))
ktrans <- readNIfTI(paste(perf, "ktrans", sep="_"))
mask <- readANALYZE(paste(perf, "mask", sep="_"))
mask <- ifelse(mask > 0, TRUE, FALSE)
mask <- mask[,,5:10]
mask <- apply(mask[,,2],1:2,sum)
x <- apply(mask,1,sum)!=0
y <- apply(mask,2,sum)!=0
...
overlay(as(dynamic[xx,yy,5:8,], "nifti"), 
        ifelse(ktrans[xx,yy,] < .1, ktrans[xx,yy,], NA),
        w=11, zlim.x=c(32,512), zlim.y=c(0,.1), plot.type="single", z=2)
dev.off()
...
xx<-37:220
overlay(as(dynamic[xx,yy,5:8,], "nifti"), 
        ifelse(Fp[xx,yy,] < .15, Fp[xx,yy,], NA),
        w=11, zlim.x=c(32,512), zlim.y=c(0,.15), plot.type="single", z=2)
dev.off()
@
...
\begin{tabular}{cc}
    $F_p$ & $\ktrans$ \\
    \includegraphics*[width=0.375\textwidth]{281949_19040721_perfusion_Fp_spline.png}&
    \includegraphics*[width=0.375\textwidth]{281949_19040721_perfusion_ktrans_spline.png}
  \end{tabular}
  \end{center}
  \caption{Left: $F_p$ parameter map computed as maximum of the response function after
  deconvolution using an adaptive Spline fit. Right: $\ktrans$ parameter map estimated
  from a response model fit.
  \label{fig:perfusion-spline}}
\end{figure}

As a summary statistic the maximum of the response function can be
used. Fig.~\ref{fig:perfusion-spline} depicts the maximum response function,
i.e., the maximum perfusion $Fp$. Alternatively, a response
model can be fitted to the response function (\code{nlr=TRUE}). Please 
note that a sample of the posterior pdf
is given for the response function and, hence, a non-linear fit to the response model
is performed for eachresponse function in the sample. \code{dcemri.spline} supports two 
models, the standard Kety model and the adiabatic approximation of
tissue homogeneity (AATH, \cite{stl-lee:aath}).  

Fig.~\ref{fig:perfusion-spline} depicts the maximum perfusion $F_p$ parameter map
for the central tumour slice. Increased perfusion is also visible in this map. 
Fig.~\ref{fig:perfusion-spline} also shows the median $\ktrans$ parameter map estimated from
fitting a Kety response model to the estimated response function. Here, compared to the 
results above, $\ktrans$ is slightly increased in top left area of the region of interest
due to the neglicance of the vascular compartment in the standard Kety model.

\pkg{dcemriS4} supports the \textit{audit trail} functionality of \pkg{oro.nifti}. 
Hence, from a parameter map given in NIfTi class, we can follow back all steps
from parameter estimation to raw signal:

<<AuditTrail,eval=FALSE>>=
print(audit.trail(fit$ktrans))
@ 
\tiny
<<AuditTrail,eval=TRUE,echo=FALSE>>=
print(audit.trail(fit$ktrans))
@ 
\normalsize
...
data. VS is supported by the LMUinnovative project BioMed-S: 
