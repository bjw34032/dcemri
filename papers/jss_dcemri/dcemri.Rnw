...
\section[]{Analysis of the RIDER Neuro MRI Data Repository}

\subsection[]{Dynamic Contrast-Enhanced MRI}

<<RIDER_Neuro_MRI>>=
perfusion <- paste("281949", "19040721", "perfusion", sep="_")
...
ca <- CA.fast(dynamic, ifelse(mask > 0, TRUE, FALSE), dangle, 
              flip, fangles, TR, verbose=TRUE)
...
mask <- readANALYZE(paste(perf, "mask2", sep="_"))
fit <- dcemri.lm(conc, (acqtimes - acqtimes[9]) / 60,
...
overlay(dyn[,,6:9,],
        ...
        w=11, zlim.x=c(32,512), zlim.y=c(.02,.15))
dev.print(png,
          paste(paste(perf, "ktrans", sep="_"), "png", sep="."),
          width=2*480, height=2*480)
@ 
<<RiderNeuroMRI+Bayes,results=FALSE,echo=TRUE>>=
fit <- dcemri.bayes(conc, (acqtimes - acqtimes[9]) / 60,
                    ifelse(mask > 0, TRUE, FALSE), model="extended",
                    aif="fritz.hansen", multicore=TRUE, verbose=TRUE)
overlay(dyn[,,6:9,],
        ifelse(fit$ktrans[,,6:9] < 0.25, fit$ktrans[,,6:9], NA),
