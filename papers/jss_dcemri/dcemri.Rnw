...
\item \code{dcemri.splines}: Non-parametric fit and deconvolution using an adaptive penalized Spline approach. Parameters can be estimated from the deconvolved response function via non-linear least squares \citep{sch-etal:TMI09}.
\end{itemize}
...
With \code{dcemri.bayes} and \code{dcemri.splines}, the posterior median
...
fit <- dcemri.lm(conc, (acqtimes - acqtimes[9]) / 60, mask, 
...
overlay(as(dynamic[,,2:5,], "nifti"), 
        ifelse(fit$ktrans[,,2:5] < 1, fit$ktrans[,,2:5], NA),
        w=11, zlim.x=c(32,512), zlim.y=c(0,.1))
dev.off()
@ 
<<RiderNeuroMRI+Bayes,eval=TRUE,echo=TRUE>>=
fit <- dcemri.bayes(conc, (acqtimes - acqtimes[9]) / 60, mask, 
                    model="extended", aif="fritz.hansen", 
                    multicore=TRUE, verbose=TRUE)
overlay(dynamic[,,2:5,],
        ifelse(fit$ktrans[,,2:5] < 1, fit$ktrans[,,2:5], NA),
        w=11, zlim.x=c(32,512), zlim.y=c(0,.125))
dev.print(png,
          paste(paste(perf, "ktrans", sep="_"), "png", sep="."),
          width=2*480, height=2*480)
@ 
\begin{figure}[!htbp]
  \begin{center}
