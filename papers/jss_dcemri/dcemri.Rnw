...
\pkg{oro.nifti} package \citep{oro.nifti}.
%, where both ANALYZE and NIfTI data are represented in S4 classes.  
...
Deviations from isotropy (anisotropic diffusion) in tissue is then
used to infer biological information; e.g., detection, disease
...
Typically, several structural sequences are performed (both T1- and
T2-weighted) after the patient has been positioned in the scanner.
...
of T1-weighted images are started approximately $30-60\;\text{sec}$
before a bolus injection of a low molecular weight contrast agent -- a
gadolinium chelate -- and continue for $8-12\;\text{min}$ in oncology
studies.  The length of the dynamic acquisition can easily exceed one
hour when characterizing the integrity of the blood brain barrier.
Temporal sampling rates depend on multiple factors: type of cancer,
anatomy, spatial resolution, spatial coverage, field strength of the
scanner, breathing method, etc. and must be carefully considered on a
study-by-study basis.
% Using a T1-weighted sequences, after injection a dynamic series of
% images is aquired.
DCE-MRI relies on the reduction in $T_1$ relaxation time caused by the
...
$I_1$ or $I_2$; i.e., $f_1(T_1,\text{TR})=f_2(T_1,\text{TR})=1$.
...
separates voxel belonging to the 10~unique gels.
...
the estimated T1~relaxation times, across all voxel in the ROI
...
inherent deficiency in parametric models regardless of application --
...
squares of the observational errors $\min\{\sum_t[\epsilon(t)]^2\}$.
The parameter $\ve$ is calculated using \eqref{eqn:vie}.
...
\begin{enumerate}
  ...
  \begin{eqnarray}
    \log(\ktrans) &\sim& N(a_{\ktrans},b_{\ktrans}), \label{priorktrans}\\
    \log(\kep) &\sim& N(a_{\kep},b_{\kep}), \label{priorkep}
  \end{eqnarray}
  ...
  use parameters $a_{\ktrans}=a_{\kep}=0$ and
  $b_{\ktrans}=b_{\kep}=1$.  Thus, the expected value of $\ktrans$ and
  $\kep$ is 1, and with $99.86\%$ probability \emph{a priori} neither
  kinetic parameter will exceed $20\,\text{min}^{-1}$.  For scans
  covering other tissue types, the hyperparameters
  $(a_{\ktrans},b_{\ktrans})$ and $(a_{\kep},b_{\kep})$ may be adjusted
  accordingly when calling \code{dcemri.bayes}.
  
  In case of the ``extended'' model, a Beta prior with parameters
  $a_{v_p}$ and $b_{v_p}$ is used for the vascular fraction $v_p$,
  with \emph{a priori} expected value
  $E(v_p)=a_{v_p}/(a_{v_p}+b_{v_p})$.
\item For the \emph{prior parameter}, in this case the variance of the
  ...
  \end{equation}
  with default parameters $a_{\sigma^2}=1$ and $b_{\sigma^2}=0.001$
...
\begin{equation}\label{bayestheorem}
  \pi(\mathbf{h}\,|\,C_t) = \frac{\pi(\mathbf{h}) ~
    \ell(C_t\,|\,\mathbf{h})}{\int\pi(\mathbf{h}^{\ast}) ~
    \ell(C_t\,|\,\mathbf{h}^{\ast})},
\end{equation}
where $\mathbf{h}=(\ktrans,\kep,\sigma^2)$ denotes the vector of all
parameters across all voxel, $\pi(\mathbf{h})$ the product of the
prior PDFs and $\ell(C_t\,|\,\mathbf{h})$ denotes the (Gaussian)
likelihood function of $C_t(t)$ from \eqref{datamodel}.  In the
Bayesian framework, conclusions are drawn from the joint posterior PDF
...
\item The function \code{dcemri.bayes} provides the posterior median
  ...
  computational efficiency draws from the posterior distribution are
...
\begin{enumerate}
  ...
  \item For the \emph{process model} a general approach is used, such
   ...
   \item The \emph{prior} for the adaptive smoothing parameter
     ...
     \begin{equation}
       \delta_i^2 \sim \text{IG}(a_{\delta},b_{\delta})
     \end{equation}
     with default parameters $a_{\delta}=b_{\delta}=10^{-5}$ that
...
assessed using a MCMC algorithm.  By default, the function
...
from an appropriate voxel or collection of voxel.  However, for
...
seconds to minutes and offset by the bolus injection time (at the
ninth acquisition).  This information was obtained from the original
DICOM data and saved in \code{rawtimes.txt}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[tbp]
  \begin{center}
    \includegraphics*[width=0.85\textwidth]{281949_19040721_perfusion_ktrans.png}
  \end{center}
  \caption{Statistical images of $\ktrans$ maps for the RIDER Neuro
    MRI data (slices $z=5:9$).  Two potential tumors are visible, in the
...
png(file=paste(paste(perf, "vp", sep="_"), "png", sep="."), 
    width=2*480, height=2*480)
overlay(as(dynamic[x,y,,], "nifti"), 
        ifelse(fit.lm$vp[x,y,] < 0.03, fit.lm$vp[x,y,], NA),
...
\cite{gil-ric-spi:MCMC} and \code{dcemri.map} uses numerical
...
a Gaussian distribution with expected value $a_{\ktrans}=\log(0.05)$
and variance $b_{\ktrans}=1$ on $\log(\ktrans)$, and a Gaussian
distribution with expected value $a_{\kep}=\log(0.7)$ and variance
$b_{\kep}$ on $\log(\kep)$ as priors.  For $\vp$, we use the Beta
distribution $B(a_{v_p}=1,b_{v_p}=19)$; i.e., the expected value is
...
@ 
<<RiderNeuroMRI+map2,echo=FALSE,results=hide>>=
...
<<RiderNeuroMRI+bayes0,eval=FALSE>>=
...
rm(fit.bayes)
@ 
<<RiderNeuroMRI+bayes2,eval=FALSE,echo=FALSE,results=hide>>=
...
overlay(as(dynamic[x,y,,], "nifti"), 
        ifelse(fit.spline$Fp[x,y,] < 0.2, fit.spline$Fp[x,y,], NA),
        z=7, w=11, zlim.x=c(32,512), zlim.y=c(0,0.2), plot.type="single")
dev.off()
@
...
@ 
\normalsize
\caption{XML-based ``audit trail'' for the dynamic acquisition of the
  RIDER Neuro MRI data.}
...
multi-dimensional array that holds the dynamic acquisition (in raw
signal intensities).  The three main blocks of activity are
\code{<created>}, \code{<saved>} and \code{<read>}.  In each block
pertinent information has been recorded; such as the function call,
version of \proglang{R} being used, version of the \pkg{oro.nifti}
package being used, user~ID, date, etc.  Notice that some of these
properties have changed over time, allowing one to accurately
reproduce the data processing stream (if necessary).
...
performed at each visit.\footnote{The analysis of DTI data is beyond
  the scope of this article, but the interested reader is pointed to
  the following references: \citet{Horsfield2002,tof:book}.}  The
methodology behind DWI and DTI are virtually identical, so we will
ignore the extra information provide by a DTI acquisition and analyze
the non-directional aspects of the diffusion process here.  There are
13 data volumes in the DWI acquisition: a single T2-weighted image
without diffusion weighting ($b=0$) and 12 volumes with different
gradient encodings but the same diffusion weighting.  Unfortunately,
the exact $b$-value is not known for these data so we will use
$b=1000$ since it is a common value in clinical practice.
...
b <- c(0, rep(1000, ntim(dwi)-1)) # this is a guess!
...
correspond to pure isotrpic diffusion of water molecules in the
...
of the indirect nature of MRI data acquisition.  However, numerous
authors have published on this topic and the interested reader is
