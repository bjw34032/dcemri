...
\subsection[]{T1 Relaxation rate and Gadolinium Concentration}
...
Figure~\ref{fig:sdam+t1-phantom} is the estimated B1+ field (with
...
\end{eqnarray}
Here, $\ktrans$ represents the volume transfer constant between
extracellular extravascular space (EES) per minute, and $\kep$ is the
rate constant between EES and blood plasma. The parameter $\vp$
describes the fraction of plasma in the tisse, and $v_e=\ktrans/kep$
can be interpreted as fraction of EES.
...
\code{dcemri.lm} and maximum a posteriori estimates in
\code{dcemri.map}.  With \code{dcemri.bayes} the posterior median is
provided as summary statistic, estimated via Markov chain Monte Carlo.
All samples from the joint posterior distribution may be provided
using \code{samples=TRUE}, allowing one to interrogate the posterior
probability density function (PDF) of the parameter estimates, which
can be used; e.g., to compute credible intervals.  Given estimated
kinetic parameters, \code{kineticModel} and \code{orton.exp.lm} can be
used to compute constrast agent concentration for specific models.
...
$\log(\kep)$, a Beta prior on $\vp$, and a Gamma prior on the variance
of the observation error are used.
...
cureve using an adaptive penalized Spline approach
\citep{sch-etal:TMI09}.  Kinetic parameters can however be estimated
by fitting the response model to the estimated response curve; e.g.,
an exponential in case of a Weinman model.  \code{dcemri.spline} also
uses a Bayesian approach; i.e., smoothing hyperpriors have to be
defined and posterior PDFs can be computed for all results.
...
\begin{figure}[!htbp]
  ...
  \caption{Statistical images of $\ktrans$ maps for the RIDER Neuro
    ...
    region-of-interest, by ehanced rims of high $\ktrans$ and central
...
centre of both rings, indicating that no blood is supplied to the
probably necrotic core of the tumours.
...
kinetic parameters.  All maps strengthen the finding of two tumours
with necrotic cores.  For both tumours the fraction of extravascular
...
perfusion in the tumours, i.e., low values of $\kep$ and $v_p$,
...
across the healthy and the tumour tissue goodness of fit is nearly
invariant.
...
<<RiderNeuroMRI+map1,eval=TRUE,echo=TRUE>>=
...
the central tumour slice of the RIDER example (bottom left slice in
...
<<RiderNeuroMRI+Spline,eval=TRUE,echo=TRUE>>=
...
is performed for eachresponse function in the sample.  The
...
parameter map for the central tumour slice. Increased perfusion is
...
the region of interest due to the neglicance of the vascular
