...
Using the endogenous contrast of water diffusion in tissue, DWI
quantifies the deviation of diffusion from isotropy; i.e., assuming
completely unrestricted motion of water (isotropic diffusion) how
restricted is the motion of water in a given voxel of tissue?
Deviations from isotropy (anisotropic diffusion) in tissue
\textcolor{BrickRed}{are} then used to infer biological information;
e.g., detection, disease progression, treatment response.
...
($b\geq500\;\text{s/mm}^2$) \citep{pad-etal:neoplasia}.  When
estimating the ADC value, one should exclude any acquisitions with
$b\leq100\;\text{s/mm}^2$ to minimize the influence of perfusion
effects.

The \textcolor{BrickRed}{diffusivity} of water without restrictions is
...
correspond to pure isotropic diffusion of water molecules in the
tissue.  Thus, bright areas in the ROI may be found, for example, in
the ventricles or major blood vessels and to a lesser extent the
tumor(s).  There appear to be areas of high diffusion in the core of
each tumor when compared to either the rim of the tumor or ``normal''
brain tissue (white or grey matter).
...
avaialble.  Point estimates for kinetic parameters are provided in a
...
reproduced in a strightforward manner and errors in the analysis may
...
The \pkg{dcemriS4} package is availabe from CRAN
