...
output in the NIfTI format.  Estimation of $T_1$ relaxation from multiple
...
The \proglang{S}4 designation in \pkg{dcemriS4} means that \proglang{S}4 object classes are
...
%, where both ANALYZE and NIfTI data are represented in \proglang{S}4 classes.  
...
Typically, several structural sequences are performed (both $T_1$- and
$T_2$-weighted) after the patient has been positioned in the scanner.
Conversion of signal intensity to contrast agent concentration
involves the preliminary step of estimating the $T_1$ relaxation value of
...
of $T_1$-weighted images are started approximately
...
% Using a $T_1$-weighted sequences, after injection a dynamic series of
% images is aquired.
...
\begin{enumerate}
  \item Pre-processing of the $T_1$ signal (e.g., correction of the B1
...
\subsection{$T_1$ Relaxation Rate and Contrast Agent Concentration}
\label{sec:t1}

Estimation of the tissue $T_1$ relaxation rate is the first step in
...
to consult known $T_1$ values $(T_{10}=1/R_{10})$ for different tissue
...
\end{equation}
A long repetition time ($\text{TR}\geq{5T_1}$) is typically used with
double-angle methods so that there is no $T_1$ dependence in either $I_1$
or $I_2$; i.e., $f_1(T_1,\text{TR})=f_2(T_1,\text{TR})=1$.  Instead,
...
same state regardless of whether the or $\alpha_2$ excitation was used
for the preceding acquisition.

\subsubsection{Example using Phantom Data}

Using data acquired from a $T_1$ phantom at two flip angles,
...
\begin{figure}[tbp]
  ...
  \caption{\textbf{(a)} Estimated B1+ field (with isotropic Gaussian
    ...
    angle ($60^\circ$).  \textbf{(b)} Estimated $T_1$ relaxation rates
...
gel-based phantom containing a variety of $T_1$ relaxation times.  The
...
used to estimate the $T_1$ relaxation rate from the subject (or phantom).
...
Figure~\ref{fig:sdam+t1-phantom}b displays the quantitative $T_1$ map for
...
$T_1\in[350,1543]\;\text{ms}$, where shorter $T_1$ relaxation times
...
\begin{figure}[tbp]
  ...
  \end{center}
  \caption{Boxplots of the estimated $T_1$ values for the gel-based
    phantom, grouped by user-specified regions of interest.  True
    $T_1$~values are displayed as colored circles for each distinct ROI.}
...
We compare the ``true'' $T_1$ values for each ROI with those obtained
...
the estimated $T_1$~relaxation times, across all voxel in the ROI
defined by \code{pmask}, with the true $T_1$ values (large circles).  The
...
13 data volumes in the DWI acquisition: a single $T_2$-weighted image
without diffusion weighting ($b=0$) and 12 volumes with different
gradient encodings but the same diffusion weighting.  The $b$-value
for this acquisition is $b=1000\;\text{sec/mm}^2$ (Barboriak,
...
packages.  Using \pkg{dcemriS4} dynamic $T_1$-weighted acquisitions are
