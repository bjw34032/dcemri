...
tools for quantitative assessment of dynamic contrast-enhanced MRI and
diffusion-weighted MRI.  Image processing is provided for the ANALYZE
and NIfTI data formats as input with all parameter estimates being
output in the NIfTI format.  Estimation of T1 relaxation from multiple
flip-angle acquisitions, using either constant or spatially-varying
flip angles, is performed via nonlinear regression.  Both
literature-based and data-driven arterial input functions are
available to be combined with a variety of compartmental models.
Kinetic parameters are obtained from nonlinear regression, Bayesian
estimation via Markov Chain Monte Carlo or Bayesian \emph{maximum a
  posteriori} estimation.  A non-parametric model, using penalized
splines, is also available to characterize the time curves.  Given the
size of multi-dimensional data sets commonly acquired in imaging
studies, care has been taken to maximize computational efficiency and
minimize memory usage.  All methods are illustrated using both
simulated and real-world medical imaging data available in the public
domain.
...
\Address{
  ...
  London W12 0HS\\
  United Kingdom\\
  E-mail: \email{b.whitcher@imperial.ac.uk}\\
...
\proglang{R} \citep{R} and exported for accessibility across a wide
variety of medical image analysis software packages.
...
\citep{oro.nifti}.
% , where both ANALYZE and NIfTI data are represented in \proglang{S}4
% classes.
Parameter estimates in \pkg{dcemriS4} inherit attributes from the
incoming ANALYZE/NIfTI objects in order to preserve anatomical and
physiological information for appropriate visualization in
\proglang{R} or other software packages.  All functions for parameter
estimation may also be applied to aggregated data; i.e., a mean curve
across an anatomical region of interest.  However, the methodology
presented here is intended to be applied on a voxel-by-voxel basis to
the ANALYZE or NIfTI objects and all statistical summaries are output
as valid NIfTI objects to facilitate interoperability.  As voxel-wise
quantitative analysis can be time consuming, \pkg{dcemriS4} supports
basic parallel computing by incorporating the \pkg{multicore} package
\citep{multicore}.  Computations are easily parallelized with the
variable \code{multicore=TRUE} in the most computationally expensive
functions.
...
oncology imaging biomarker \citep{che-etal:diffusion,koh-pad:DW-MRI}.
DWI allows one to quantify the diffusion behavior of water by
estimating the apparent diffusion coefficient (ADC)
\citep{whe-etal:D}.  Assuming completely unrestricted motion of water
how restricted is the motion of water given the barriers of biological
tissue?  \textcolor{BrickRed}{The reduction in the ability of water to
  diffuse in tissue has been are} used to infer biological
information; e.g., tumor detection, disease progression, treatment
response.
...
% Typically, several structural sequences are performed (both T1-
% and T2-weighted) after the patient has been positioned in the
% scanner.
Conversion of signal intensity to contrast agent concentration
involves the preliminary step of estimating the T1 relaxation value of
the tissue; e.g., using multiple flip-angles or inversion recovery.
For higher field strengths one may also consider characterizing the
inhomogeneity of the magnetic field in the scanner.  The dynamic set
of T1-weighted images are started approximately
$30-60\;\text{seconds}$ before a bolus injection of a low molecular
weight contrast agent -- a gadolinium chelate -- and continue for
$8-12\;\text{min}$ in oncology studies.  The length of the dynamic
acquisition can easily exceed one hour when characterizing the
integrity of the blood brain barrier.  Temporal sampling rates depend
on multiple factors: type of cancer, anatomy, spatial resolution,
spatial coverage, field strength of the scanner, breathing method,
etc. and must be carefully considered on a study-by-study basis.
% Using a T1-weighted sequences, after injection a dynamic series of
% images is aquired.
DCE-MRI relies on the reduction in T1 relaxation time caused by the
contrast agent.  The quantitative analysis of DCE-MRI data in
\pkg{dcemriS4} consists of the following steps
\begin{enumerate}
  \item Pre-processing of the T1 signal (e.g., correction of the B1
    field, motion correction, co-registration) is introduced in
...
\citep{buc-par:measuring}.  \textcolor{BrickRed}{Note, the
  longitudinal (or spin-lattice) relaxation time T1 is the decay
  constant of the recovery of the $z$ component of the nuclear spin
  magnetization towards its thermal equilibrium value
  \citep{bux:introduction}.}  Multiple flip-angle acquisitions are
commonly used to estimate the intrinsic relaxation rate maps
$\{m_0,R_{10}\}$ of the tissue, where $m_0$ is \textbf{???} and
$R_{10}$ is the pre-injection longitudinal relaxation rate.  The
non-linear equation
\begin{equation}
...
flip angle $\theta$ \textcolor{BrickRed}{before injection of the
  contrast agent.}  Note, a repetition time of
$\TR\approx4\;\text{ms}$ is common practice for pulse sequences in
clinical applications.  The Levenberg-Marquardt algorithm, provided in
\pkg{minpack.lm} \citep{minpack.lm}, is applied to estimate the
parameters; see the discussion by \cite{ahe-etal:levenburg-marquardt}.
It is worthwhile to consult known T1 values $(T_{10}=1/R_{10})$ for
different tissue types \textcolor{BrickRed}{(e.g., muscle, grey
  matter, white matter)} to ensure the parameter estimates obtained
are sensible.
...
algorithm.  The final step of the conversion of the dynamic signal
...
\url{http://www.itk.org}) is highly desirable and currently under
investigation.
...
\textcolor{BrickRed}{The B1 field is the magnetic field created by the
  radio frequency coil passing an alternating current at the Larmor
  frequency.}  The B1+ field is the transverse, circularly polarized
component of B1 that is rotating in the same sense as the
magnetization.  When exciting large collections of spins,
non-uniformity in B1+ results in nonuniform treatment of spins.  This
leads to a spatially varying signal and contrast, and to difficulty in
image interpretation and quantification \citep{cun-pau-nay:saturated}.
...
\textcolor{BrickRed}{The focus in this section is fully quantitative
  pharmacokinetic modeling of tissue perfusion and assumes the raw
  scanner data has been converted to contrast agent concentration.
  Please see \cite{col-pad:ieee} and \cite{buc-par:measuring} for
...
The parameter $\vp$ describes the fraction of contrast agent in the
plasma, while 
...
\begin{enumerate}
  \item For the \emph{data model} we assume a signal-plus-noise
    model; such that the observed concentration of contrast agent
    $C_t(t)$ in a single voxel at time point $t$ with additive
    Gaussian error variance $\sigma^2$, is given by
  ...
  reasonable values for both parameters should not exceed values of
  ...
  In case of the ``extended'' model, a Beta prior with parameters
  $a(v_p)$ and $b(v_p)$ is used for the vascular fraction $v_p$,
  with \emph{a priori} expected value
...
\item The function \code{dcemri.bayes} provides the posterior median
  ...
  \citep{gil-ric-spi:MCMC}.  All samples from the joint posterior
  distribution may be saved using the option \code{samples=TRUE},
  allowing one to interrogate the posterior probability density
  function (PDF) of all parameter estimates.  To increase
  computational efficiency sampling from the posterior distribution is
  implemented in \proglang{C} and linked to \proglang{R}.  We have
  found retaining all samples from the joint posterior to be useful
  when wanting to construct, for example, voxel-wise credible
  intervals on the kinetic parameters.  The algorithm is
  computationally expensive.  Parallel computation is enabled with the
  \code{multicore} package \citep{multicore} by setting the option
  \code{multicore=TRUE}.
\end{itemize}
...
Spline approach \citep{sch-etal:TMI09}.  A hierarchical Bayesian model
is constructed
\begin{enumerate}
...
(from a bolus injection) is also available.  The algorithm follows
these steps:
\begin{enumerate}
...
homogeneity model (\code{"AATH"}) with response function
\begin{equation}
  R(t) = F_p \cdot \left\{
  \begin{array}{ll}
    E \exp\left[-\left(t-T_c\right)EF_p/\ve\right] & \text{for} \quad t \geq T_c,\\
    0 & \text{for} \quad t < 0,\\
    1 & \text{for} \quad 0 \leq t < T_c,
  \end{array} \right.
\end{equation}
where $T_c$ is the transit time through the capillary, $E$ is the
extraction fraction and $F_p$ is the mean plasma flow. These parameters 
translate into parameters from the (extended) Kety model as follows
\begin{eqnarray}
  \ktrans &=& EF_p,\\
  \kep &=& EF_p/\ve,\\
  \vp &=& T_cF_p.
...
curve, and median and standard error of the kinetic parameters are
...
(\code{samples=TRUE}).  This algorithm is computationally expensive,
and parallel computing may be accessed using the \code{multicore}
package (\code{multicore=TRUE}).
...
development in the software project
\pkg{PILFER}.\footnote{\url{http://pilfer.sourceforge.net}}  One may
also question the rationale for hypothesis testing in only one kinetic
...
(DW-MRI), is a technique \textcolor{BrickRed}{that provides a unique
  insight into tissue structure through MRI diffusion measurements}
\emph{in vivo} \citep{mos-etal:diffusion,whe-etal:D}.  Contrast is
...
to any particular direction (isotropic diffusion).  
...
The \textcolor{BrickRed}{diffusivity} of water \textcolor{BrickRed}{at
  room temperature} without restrictions is approximately
$3.0{\times}10^{-3}\;\text{mm}^2/\text{s}$.  Once the ADC is estimated
in the tissue of interest at baseline, treatment response may be
...
apoptosis after treatment \citep{koh-pad:DW-MRI,pad-etal:neoplasia}.
...
DCE-MRI and DW-MRI data analyzed here were downloaded from the
``RIDER~Neuro~MRI''
...
estimate the intrinsic relaxation rate $R_{10}$ and scaling factor
$m_0$ from \eqref{eqn:signal} and the contrast agent concentration
curve $C_t(t)$ from \eqref{eqn:conc}.  In order to save computation
time and memory, we utilize a binary mask with a very limited
region-of-interest (ROI) created in
FSLView\footnote{\url{http://fsl.fmrib.ox.ac.uk/fsl/fslview/}} and
saved in ANALYZE format.
...
\eqref{eqn:extendedkety}.  It is relatively straightforward to
...
\code{rawtimes.txt}.  \textcolor{BrickRed}{The literature-based AIF
  \code{fritz.hansen} is used here for illustrative purposes.
  Alternatives include estimating values for a parametric AIF (e.g.,
  \code{aif="orton.exp"}) from the data and supplying them via the
  \code{user} option in \code{dcemri.lm}, or providing an empirical
  AIF (\code{aif="empirical"}) and passing the vector of AIF values
  via the \code{user} option.}
...
overlay(dynamic, ifelse(mask, fit.lm$ktrans, NA), w=11, zlim.x=c(32,512), 
        zlim.y=c(0,0.1))
@ 
<<RiderNeuroMRI+lm6,eval=FALSE,echo=FALSE,results=hide>>=
dev.off()
...
overlay(as(dynamic[x,y,,], "nifti"), 
        ...
        zlim.x=c(32,512), zlim.y=c(0,0.1), plot.type="single")
dev.off()
png(file=paste(paste(perf, "kep", sep="_"), "png", sep="."), 
    width=2*480, height=2*480)
overlay(as(dynamic[x,y,,], "nifti"), 
        ifelse(mask[x,y,], fit.lm$kep[x,y,], NA), z=7, w=11, 
        zlim.x=c(32,512), zlim.y=c(0,1.25), plot.type="single")
dev.off()
...
Markov Chain Monte Carlo (MCMC) to explore the posterior~PDF
...
<<RiderNeuroMRI+map4,eval=FALSE,echo=FALSE,results=hide>>=
...
parameters the number of voxel where the MAP estimator does not
...
rm(fit.bayes)
@ 
<<RiderNeuroMRI+bayes3,eval=FALSE,echo=FALSE,results=hide>>=
...
overlay(as(dynamic[x,y,,], "nifti"), 
        ...
        zlim.x=c(32,512), zlim.y=c(0,0.0075), plot.type="single")
dev.off()
...
the majority of non-tumor voxel in the ROI.  However, $\ktrans$
...
However, $\text{sd}(\ktrans)$ is quite low overall, usual less than
...
splines \citep{Eilers1996,Marx1998}.  Smoothness of the curve and
...
(\code{ab.tauepsilon}).  Full details on the methodology for Bayesian
penalized $P$-splines are provided in \cite{sch-etal:TMI09}.  For the
following application of \code{dcemri.spline} default values for the
...
@ 
<<RiderNeuroMRI+spline2,eval=FALSE,echo=FALSE,results=hide>>=
...
overlay(as(dynamic[x,y,,], "nifti"), 
        ...
        zlim.x=c(32,512), zlim.y=c(0,0.2), plot.type="single")
dev.off()
@
...
function, i.e., the maximum perfusion $F_p$.  Alternatively, a response
...
image is poor -- most likely due to the fact that the acquisition
protocol was not optimized for the AATH model.
...
$\ktrans$ is slightly increased in top left area of the region of
interest due to the negligence of the vascular compartment in the
standard Kety model.
...
performed at each visit.  \textcolor{BrickRed}{The analysis of DTI
  data is beyond the scope of this article, but the interested reader
  is pointed to the following references:
...
provided by a DTI acquisition and analyze the non-directional aspects
of the diffusion process here.  \textcolor{BrickRed}{We acknowledge
  the fact that the ADC values derived from this DTI acquisition may
  differ slightly from those estimated using a more common DWI
  sequence.}
...
$b$-value for this acquisition is $b=1000\;\text{sec/mm}^2$
(Barboriak, \emph{personal communication}), a common value in clinical
practice.  As previously noted this acquisition protocol has not been
optimized for ADC estimation, and will include both perfusion and
...
overlay(as(dwi[,,17:20,], "nifti"),
        ifelse(tmask[,,17:20], fit.adc$D[,,17:20], NA), zlim.x=c(32,2048), 
...
correspond \textcolor{BrickRed}{to the high mobility of water
  molecules in tissue}.  Thus, bright areas in the ROI may be found,
for example, in the ventricles or major blood vessels and to a lesser
extent the tumor(s).  There appear to be areas of high diffusion in
the core of each tumor when compared to either the rim of the tumor or
``normal'' brain tissue (white or grey matter).
...
of the indirect nature of MRI data acquisition;
\textcolor{BrickRed}{the pharmcodynamic effects of treatment are being
  measured not directly in tissue, but via the diffusivity of water
  molecules in the tissue}.  However, numerous authors have published
on this topic and the interested reader is encouraged to look at
...
@ 
\normalsize
\caption{XML-based ``audit trail'' for the diffusion-weighted
  acquisition of the RIDER~Neuro~MRI data.}
...
Figure~\ref{fig:audit-trail} displays the audit trail for the
multi-dimensional array that holds the diffusion-weighted acquisition
(in raw signal intensities).  The main blocks of activity are
\code{<created>}, \code{<saved>}, \code{<read>} and \code{<event>}.
The first two tags occurred during the initial DICOM-to-NIfTI
conversion process and the last three tags were performed when
compiling this Sweave document.  In each block pertinent information
has been recorded; such as the function call, version of \proglang{R}
...
scanner, assumed to be provided in DICOM format, may be converted to
the NIfTI format using the \pkg{oro.dicom} and \pkg{oro.nifti}
packages.  Using \pkg{dcemriS4} dynamic T1-weighted acquisitions are
converted into contrast agent concentration time curves on a
voxel-by-voxel basis.  A variety of compartmental models for the
tissue kinetics, and models for the arterial input function (AIF), are
available.  Point estimates for kinetic parameters are provided in a
fast and robust manner using either least-squares or \emph{maximum a
  posteriori} techniques, and information about the uncertainty in
these parameter estimates may be obtained from the Bayesian MCMC
(Markov Chain Monte Carlo) algorithm; e.g., by looking at standard
errors, credible intervals or the entire posterior distribution.
...
data analysis pipeline may be recorded using the audit trail
capability provided by \pkg{oro.nifti}.  Hence, results may be
reproduced in a straightforward manner and errors in the analysis may
be identified efficiently.

% To our knowledge, \pkg{dcemriS4} is the only freely available software
% package for analysis of DCE-MRI and DWI data \textbf{???}. 
...
medical imaging data to the public domain.  VS is supported by the LMU
innovative project BioMed-S: Analysis and Modelling of Complex
Systems.
