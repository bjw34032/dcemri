...
output in NIfTI format.  Estimation of T1 relaxation from multiple
...
available and may be combined with a variety of compartmental models.
...
splines, is also available to characterize the contrast agent
concentration time curves.  Given the size of multi-dimensional data
sets commonly acquired in imaging studies, care has been taken to
maximize computational efficiency and minimize memory usage.  All
methods are illustrated using both simulated and real-world medical
...
\textcolor{BrickRed}{While initial application of DCE-MRI was in the
  characterization of blood-brain barrier (BBB) integrity
  ...
  vaccine therapies, are expected to be cytostatic (i.e., inhibiting
  or suppressing cellular growth) and may not produce changes in tumor
  size as measured by traditional structural imaging techniques
  \citep{cho-dwy-kno:functional}.  Thus, the ability to study
  malignant vasculature through non-invasive MRI methods is
  advantageous.  We will cover a DCE-MRI data analysis pipeline using
  \pkg{dcemriS4} that is fully quantitative and produces
  biologically-relevant parmameter estimates from a compartmental
  model.  The interested reader will find several chapters in
  \citet{jac-buc-par:dce-mri} and \citet{par-pad:DCE-MRI} pertaining
  to the scientific background, methodology and application of
  DCE-MRI.}
...
% images is aquired.
DCE-MRI relies on the reduction in T1 relaxation time, corresponding
to positive signal enhancement, caused by the presence of the contrast
agent.  The quantitative analysis of DCE-MRI data in \pkg{dcemriS4}
...
$\{m_0,R_{10}\}$ of the tissue, where $m_0$ is the equilibrium signal
intensity and $R_{10}$ is the pre-injection longitudinal relaxation
rate.  The non-linear equation
\begin{equation}
...
\textcolor{BrickRed}{Diffusion weighted imaging (DWI) is under rapid
  development as an oncology imaging biomarker
  \citep{che-etal:diffusion,koh-pad:DW-MRI}.  DWI allows one to
  quantify the diffusion behavior of water by estimating the apparent
  diffusion coefficient (ADC) \citep{whe-etal:D}.  Assuming completely
  unrestricted motion of water how restricted is the motion of water
  given the barriers of biological tissue?  The reduction in the
  ability of water to diffuse in tissue has been used to infer
  biological information in oncology; e.g., in tumor detection,
  disease progression and the evaluation of treatment response.}

DWI is an MR technique \textcolor{BrickRed}{that provides a unique
...
dev.off()
@  
<<RiderNeuroMRI+lm7,echo=FALSE,results=hide>>=
...
<<RiderNeuroMRI+lm8,eval=FALSE,echo=FALSE,results=hide>>=
zrx <- c(32,256)
...
used.  
% Figure~\ref{fig:perfusion-spline} depicts the maximum response
% function; i.e., the maximum perfusion $F_p$.  
Alternatively, a response model may be derived from the response
function (\code{nlr=TRUE}).  Please note that a sample of the
...
``normal'' brain tissue (white or grey matter).  This may indicate
sparse cell density in the tumor ``cores'' due to necrotic or
apoptotic processes and the subsequent removal of cells in the tissue.
