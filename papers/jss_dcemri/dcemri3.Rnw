...
\citep{oro.nifti,neuroimageing_with_R}.
...
fully quantitative and produces biologically-relevant parmameter
...
flip angle $\theta$ before injection of the contrast agent.  Note, a
repetition time of $\TR\approx4\;\text{ms}$ is common practice for
pulse sequences in clinical applications.  The Levenberg-Marquardt
...
implementatino is under development in the software project
...
tissue?  The reduction in the ability of water to diffuse in tissue
has been used to infer biological information in oncology; e.g., in
tumor detection, disease progression and the evaluation of treatment
response.

DWI is an MR technique hat provides a unique insight into tissue
structure through MRI diffusion measurements \emph{in vivo}
\citep{mos-etal:diffusion,whe-etal:D}.  Contrast is generated when the
diffusion of molecules in tissue prefer a specific direction in
three-dimensional space (anisotropic diffusion) relative to any
particular direction (isotropic diffusion).  Using the
