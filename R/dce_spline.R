...
ddd<-3
if (length(dims)==0){dims<-c(1,1,1);ddd<-0}
if (length(dims)==1){dims<-c(1,1,dims);ddd<-1}
if (length(dims)==2){dims<-c(1,dims);ddd<-2}
...
result<-.C("dce_spline_run",as.integer(1),as.integer(burnin),as.integer(c(dims,T)),
as.double(data),as.double(tau),as.double(tauepsilon),as.double(D),
as.integer(rw),as.double(beta),as.double(c(hyper.a,hyper.b,epsilon.a,epsilon.b)),as.integer(p),as.double(1:T),as.double(1:T),as.double(1:T),
as.double(1:(T^2)),as.double(1:(T^2)),as.double(1:(T^2)),as.double(1:(T^2)),
as.double(t(D)),as.integer(silent), PACKAGE="dcemri")
...
result<-.C("dce_spline_run",as.integer(nriters),as.integer(thin),as.integer(c(dims,T)),
as.double(data),as.double(tau),as.double(tauepsilon),as.double(D),
as.integer(rw),as.double(beta),as.double(c(hyper.a,hyper.b,epsilon.a,epsilon.b)),as.integer(p),as.double(1:T),as.double(1:T),as.double(1:T),
as.double(1:(T^2)),as.double(1:(T^2)),as.double(1:(T^2)),as.double(1:(T^2)),
as.double(t(D)),as.integer(silent), PACKAGE="dcemri")
...
erg<-erg*F
if (TC< 0)erg<-rep(-10^16,length(time))
#erg<-expression(erg)
eval(erg)
}
...
for (m in 1:Z)
{
if (nlr=="med")me5<-apply(beta[i,j,m,,],1,median)%*%t(B)
if (nlr=="mean")me5<-apply(beta[i,j,m,,],1,mean)%*%t(B)
kt<-max(me5)
start<-which(kt==me5)
...
result<-list(fp.hat=MAX,tau=tau,beta=beta,tauepsilon=tauepsilon,A=A,B=B,D=D,knots=knots,estimates=parhat,fp.hat.samples=MAX0)
...
rest<-dce.spline(data[1:25+tau-1],time[1:25+tau-1]-t0,aif2,time.input2,nlr=FALSE,knotpoints=30,rw=2,nriters=500,hyper.a=1e-10,hyper.b=1e-10,epsilon.b=1e-10,silent=TRUE)
...
t0<-(b$coefficients[1]-a$coefficients[1])/(a$coefficients[2]-b$coefficients[2])

if (do.plot){
plot(tt,bb,xlab="delay in sec",ylab="MBF")
...
tau<-min(which(time[1:25]>t0))
rest<-dce.spline(t(data[1:25+tau-1]),time[1:25+tau-1]-t0,aif2,time.input2,nlr=FALSE,knotpoints=30,rw=2,nriters=500,hyper.a=1e-10,hyper.b=1e-10,epsilon.b=1e-10,silent=TRUE)
beta<-apply(rest$beta,c(3,4),median)
bc=max(rest$B%*%beta[1,])
if(do.plot)plot(time.input2,rest$B%*%beta[1,],type="l",xlab="time in sec",ylab="impulse response function")

return(list("mbf"=bc,"t0"=t0,"fit"=rest))
}
