.do.dcemri.lm <- function(list, model, aif, multicore) {
  dclm <- (dcemri.lm(list$conc, list$time, list$mask, model=model, aif=aif,
                     multicore=multicore))
  return(dclm)
}

.lomis.sample <- function(i, sample, nriters, nrscans, cons, maxvalue,
                          times, I, T, A1, M1, A2, M2, mcmc.control,
                          prior.control, nrcovar, fixedeffects, xcovar,
                          model.b, vpupdate) {
  ...
  for (i in 1:nriters) {
    ...
    for (i in 1:10) {
      results[[i]] <- c(results[[i]], as.vector(sample[[i + 27]]))
    }
  }
  return(results)
}

lomis <- function(scans, masks, timelines, scandata, 
                  fixed=c(1,"post","post*treatment"), fixed.linear=NULL,
                  random=c("patient","patient*post*treatment"),
                  ...
                  mcmc.control=list(nriters=100000, burnin=20000,
                    thinning=100, retunecycles=3, tunepct=10,
                    tuning=413, chains=20),
                  multicore=require("parallel"), verbose=FALSE) {
  if (multicore) {
    require("parallel")
  }
  ...
  for (i in 1:nrscans) {
    ...
    firsttunelist[[i]] <- list(conc = scans[[i]],
                               mask = masks[[i]],
                               time = timelines[[i]])
  }
  xcovar <- NULL
  ...
  for (i in 1:length(fixed)) {
    ...
    if (length(w) == 0) {
      ...
      for (j in 1:length(names(scandata))) {
        if (grepl(names(scandata)[j], fixed[i])) {
          w <- c(w,j)
        }
      }
  ...
  if(!is.null(fixed.linear))
    for (i in 1:length(fixed.linear)) {
      ...
      if (length(w) == 0) {
        w <- NULL
        for (j in 1:length(names(scandata))) {
          if(grepl(names(scandata)[j], fixed.linear[i])) {
            w <- c(w,j)
          }
        }
      }
  ...
  for (i in 1:length(random)) {
    ...
    if(length(w)==0) {
      w<-NULL
      for (j in 1:length(names(scandata))) {
        if (grepl(names(scandata)[j], random[i])) {
          w <- c(w,j)
        }
      }
      x0 <- apply(scandata[,w], 1, prod)
  ...
  if(!is.null(random.linear))
    for (i in 1:length(random.linear)) {
      ...
      if (length(w) == 0) {
        w <- NULL
        for (j in 1:length(names(scandata))) {
          if (grepl(names(scandata)[j], random.linear[i])) {
            w <- c(w,j)
          }
        }
      }
  ...
  if (is.null(mcmc.control$nriters))
    mcmc.control$nriters <- 100000
  if (is.null(mcmc.control$burnin))
    mcmc.control$burnin <- 20000
  if (is.null(mcmc.control$thinning))
    mcmc.control$thinning <- 100
  if (is.null(mcmc.control$tunepct))
    mcmc.control$tunepct <- 10
  if (is.null(mcmc.control$retunecycles))
    mcmc.control$retunecycles <- 3
  if (is.null(mcmc.control$tuning))
    mcmc.control$tuning <- 3
  
  nrcovar <- dim(xcovar)[2]
  
  cat("Data Prepared at ", proc.time(), fill=TRUE)
  cat("Estimating initial values..")
  ## First Step: Estimate Ktrans values per voxel
  ## ToDo: changes for user-specific AIF
  require("minpack.lm")
  if (multicore) {
    firsttunelist <- mclapply(firsttunelist, .do.dcemri.lm, model, aif,
                              multicore=FALSE, mc.preschedule=FALSE)
  } else {
    firsttunelist <- lapply(firsttunelist, .do.dcemri.lm, model, aif,
                            multicore=FALSE)
  }

  print(firsttunelist[[1]])

  cat(".done at ", proc.time(), fill=TRUE)
  cat("Initital tuning..")
  ## Step 2: Tuning
  ...
  for (i in 1:nrscans) {
    ...
    if (vpupdate == 1) {
      vp0 <- as.vector(firsttunelist[[i]]$vp)
    }
  ...
  initial<-.C("baymem_main",
              ...
              as.double(rep(prior.control$beta[1] / prior.control$beta[2],
                            dim(xcovar)[2])),
              as.double(rep(prior.control$beta[1] / prior.control$beta[2],
                            dim(xcovar)[2])),
              as.double(rep(prior.control$gamma[1] / prior.control$gamma[2],
                            nrscans)), #30
              as.double(rep(prior.control$theta[1] / prior.control$theta[2],
                            nrscans)),
              as.double(rep(prior.control$epsilon[1] / prior.control$epsilon[2],
                            nrscans)),
  ...
  if (multicore) {
    todo <- 1:cores
    samples <- mclapply(todo, .lomis.sample, sample, nriters, nrscans, cons,
                        maxvalue, times, I, T, A1, M1, A2, M2, mcmc.control,
                        prior.control, nrcovar, fixedeffects, xcovar, model.b,
                        vpupdate)
    results <- vector("list", 10)
    for (i in todo) {
      for (j in 1:10) {
    ...
    results <- .lomis.sample(0, sample, nriters, nrscans, cons, maxvalue,
                             times, I, T, A1, M1, A2, M2, mcmc.control,
                             prior.control, nrcovar, fixedeffects, xcovar,
                             model.b, vpupdate)
  }
  ...
  cat("..done at", proc.time(), fill=TRUE)
  cat("Sorry, reconstruction of results not implemented yet.")
  ## end lomis()
  return(results)
}
